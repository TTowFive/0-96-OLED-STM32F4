...
void oled_i2c_init( I2C_TypeDef * I2Cx, u16 i2c_clk, u8 own_addr ) {
    ...
    show_str( (u8*)"Hello world", FONT_8x16, 0, 0 );
...
*   i2c check event
****************************/
ErrorStatus i2c_check_event( I2C_TypeDef * I2Cx, uint32_t I2C_EVENT ) {
    u16 timeout = 0xFFFF;
    ErrorStatus es = ERROR;
    do {
        es = I2C_CheckEvent( I2Cx, I2C_EVENT );
    } while ( es == ERROR && --timeout );
    return es;
}
...
void oled_i2c_start( I2C_TypeDef * I2Cx ) {
    ...
    if ( i2c_check_event( I2Cx, I2C_EVENT_MASTER_MODE_SELECT ) == ERROR ) {
...
void oled_i2c_send_byte( u8 byte ) {
    OLED_I2C->DR = byte;
    if ( i2c_check_event( OLED_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTING ) == ERROR ) {
...
void oled_send( u8 slave_addr, u8 dat, OLED_OPT opt ) {
    ...
    if ( i2c_check_event( OLED_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED ) == ERROR ) {
